...
// data-id값으로 배열을 탐색하여 일치하는 객체가 들어있는 인덱스 변환
...
function removeTodoData($delTarget) {
  ...
  // 배열 내에 있는 객체도 삭제를 진행.
  // 삭제되는 객체가 배열 안에 몇번째 인지를 확인 -> 할 일 완료 처리 함수 쪽에 비슷한 로직이 있습니다.
  // 함수화 시켜보세요.
...
// 수정 완료 이벤트 처리 함수
function modyfyTodoData($modCompleteSpan) {
...
(function() {
  ...
  $todoList.addEventListener('click', e => {
    if(e.target.matches('.todo-list .modify span.lnr-undo')) {
      ...
      modyfyTodoData(e.target); // 수정모드에서 수정을 확정지으려는 이벤트
    }
  });
